...
- (void)initViews
{
    ...
    radius = (view_w - 6)/2;//半径
    
    UIBezierPath *path = [UIBezierPath bezierPathWithArcCenter:center radius:radius startAngle:M_PI endAngle:M_PI/1.2 clockwise:YES];//顺时针绘制
    path.lineCapStyle = kCGLineCapRound; //线条拐角
    path.lineJoinStyle = kCGLineCapRound; //终点处理
    
    shapeLayer = [CAShapeLayer layer];
    shapeLayer.path = path.CGPath;
    shapeLayer.strokeColor = [UIColor blueColor].CGColor;//线条颜色
...
-(void)setInnerRadius:(int)innerRadius innerColor:(UIColor *)innerColor outerColor:(UIColor *)outerColor{
