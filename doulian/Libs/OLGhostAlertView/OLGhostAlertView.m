...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _position = OLGhostAlertViewPositionCenter;
...
//        self.layer.shadowRadius = 30.0;
        ...
        [self setImage:[[UIImage imageNamed:imageName] stretchableImageWithLeftCapWidth:30 topCapHeight:0]];
//        [self setImage:[UIImage imageNamed:imageName] ];
        
        UIMotionEffectGroup *motionEffects = [UIMotionEffectGroup new];
        
        UIInterpolatingMotionEffect *horizontalMotionEffect = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.x" type:UIInterpolatingMotionEffectTypeTiltAlongHorizontalAxis];
        horizontalMotionEffect.minimumRelativeValue = @-114;
        horizontalMotionEffect.maximumRelativeValue = @114;
        ...
        _titleLabel.textColor = [UIColor whiteColor];
        _titleLabel.textAlignment = NSTextAlignmentCenter;
        _titleLabel.font = [UIFont boldSystemFontOfSize:TITLE_FONT_SIZE-1];
        _titleLabel.numberOfLines = 0;
        _titleLabel.lineBreakMode = NSLineBreakByWordWrapping;
        _titleLabel.autoresizingMask = (UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight);
...
- (void)showWithAnimation
{
    ...
    [self showAnimationInView:parentView];
}
...
- (void)showAnimationInView:(UIView *)view
{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        [self setFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y-60, self.frame.size.width, self.frame.size.height)];
        self.alpha = 1;
    } completion:^(BOOL finished) {
        [self performSelector:@selector(hide) withObject:nil afterDelay:self.timeout];
    }];
...
{
    if (style == OLGhostAlertViewStyleDefault) style = OLGhostAlertViewStyleDark;
