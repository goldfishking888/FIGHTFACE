...
@property (nonatomic, strong) UIImageView *specialItemBackImage;
        ...
        for (NSInteger index = 0; index < self.items.count; index++) {
          ...
          //  if (index == self.items.count - 1) {
            ...
            if (self.specialItem && self.specialItem.index == index) {
                ...
                self.specialItem.frame = CGRectMake(x, -5, width, height);
                self.specialItemBackImage.frame = CGRectMake(x, -5, 50, 50);
                self.specialItemBackImage.center = self.specialItem.center;
                x += width;
            }
        }
    }
...
- (void)setSpecialItem:(YPTabItem *)item afterItemWithIndex:(NSInteger)index backImageName:(NSString *)imageName tapHandler:(void (^)(YPTabItem *item))handler {
    ...
    self.specialItemBackImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:imageName]];
    [self addSubview:self.specialItemBackImage];
    [self addSubview:item];
    [self updateItemsFrame];
...
//更改中间按钮图片
- (void)setSpecialItemImage:(NSString *)imageName {
    self.specialItem.image = [UIImage imageNamed:imageName];

}

//更改中间按钮背景
- (void)setSpecialItemBackGroundImage:(NSString *)imageName isAnimated:(BOOL)isAnimated{
    self.specialItemBackImage.image = [UIImage imageNamed:imageName];
    
    if (isAnimated) {
        [self.specialItemBackImage.layer removeAllAnimations];
        ...
        [self.specialItemBackImage.layer addAnimation:rotationAnimation forKey:[NSString stringWithFormat:@"%@",imageName]];
    }
}
