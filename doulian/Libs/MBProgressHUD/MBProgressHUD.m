...
-(void)detailLabelClick{
    if ([delegate respondsToSelector:@selector(hudDetailLabelClick)]) {
        [delegate performSelector:@selector(hudDetailLabelClick) withObject:self];
    }
}
...
- (void)setupLabels {
    ...
    [detailsLabel setUserInteractionEnabled:YES];
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(detailLabelClick)];
    [detailsLabel addGestureRecognizer:tap];
