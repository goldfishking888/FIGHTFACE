...
@property(nonatomic,assign)NSString *isBirthday;
...
-(instancetype)initDatePickWithDate:(NSDate *)defaulDate datePickerMode:(UIDatePickerMode)datePickerMode isHaveNavControler:(BOOL)isHaveNavControler withIsBirth:(NSString *)isBirth{
...
-(void)setUpPickView{
    ...
    pickView.frame=CGRectMake(0, 0, pickView.frame.size.width, pickView.frame.size.height + 50);
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode{
    ...
    if ([_isBirthday isEqualToString:@"1"])
    {
...
-(UIToolbar *)setToolbarStyle
{
    ...
    UIBarButtonItem *left=[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"取消", nil) style:UIBarButtonItemStylePlain target:self action:@selector(cancelClick)];
    left.tintColor = [UIColor whiteColor];
    
    UIBarButtonItem *centerSpace=[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:self action:nil];
    
    UIBarButtonItem *right=[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"完成", nil) style:UIBarButtonItemStylePlain target:self action:@selector(doneClick)];
    right.tintColor = [UIColor whiteColor];
    toolbar.items=@[left, centerSpace, right];
    return toolbar;
}
-(void)setToolbarWithPickViewFrame{
    _toolbar.frame=CGRectMake(0, 0,[UIScreen mainScreen].bounds.size.width, 30);
}
...
//        NSString *dateStr = [formater stringFromDate:_defaulDate];
//        
